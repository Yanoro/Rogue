...
void Game::SystemRenderDrawAscii(const DrawAscii &ascii, const ScreenPosition screenPos) {
    DrawRectangleV(
...
static float GetDistanceRecs(Rectangle rec1, Rectangle rec2) {
...
void Game::SystemApplyFriction(Velocity &vel, Acceleration &accel, const Friction &friction) {
    if (vel.Length() > 3.0f) {
        float gravity = 9.8f;
...
void Game::SystemSteerTargetPath(flecs::entity entity, Velocity &vel, Acceleration &accel, const GamePosition &currPos, const ScreenPosition &screenPos, const Hitbox &hitbox, TargetPath &tPath) {
    if (tPath.path.empty()) {
...
void Game::SystemApplyAcceleration(Velocity &vel, const Acceleration &accel) {
    vel += accel * GetFrameTime();
}

void Game::SystemApplyMaxSpeed(Velocity &vel, const MaxSpeed &maxSpeed) {
    vel = vel.Clamp(0.0f, maxSpeed.value);
}

void Game::SystemApplyCollision(Velocity &vel, const Hitbox &hitbox, const ScreenPosition &origScreenPos) {
...
void Game::SystemApplyVelocity(const Velocity &vel, ScreenPosition &pos) {
    pos += vel * GetFrameTime();
}

void Game::SystemResetAcceleration(Acceleration &accel) {
    lastAccel = accel;
    accel = {};
}

void Game::SystemSaveVelocity(const Velocity &vel) {
    lastVel = vel;
}

void Game::SystemUpdateGamePosition(const ScreenPosition &screenPos, GamePosition &gamePos) {
